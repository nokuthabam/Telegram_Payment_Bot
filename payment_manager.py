...
- Converts USD → crypto via CoinGecko free API
"""
...
DECIMALS = {
    "BTC": 8, "ETH": 6, "USDT_ERC20": 2, "USDT_TRC20": 2,
    "SOL": 4, "LTC": 6, "TRX": 2,
}
...
class PaymentManager:
    ...
    def generate_unique_usd_amount(self, base_amount: float, coin_key: str, db) -> tuple[float, float]:
        """
        ...
        for cents in possible_adjustments:
            ...
            final_amount = round(base_amount + adjustment, 2)
    ...
    async def convert_usd_to_crypto(self, usd_amount: float, coin_key: str) -> str:
        """Fetch live price from CoinGecko and convert USD → crypto."""
        # USDT is always ~$1
        if coin_key in ("USDT_ERC20", "USDT_TRC20"):
        ...
        try:
            ...
            logger.warning(f"Price fetch failed for {coin_key}: {e}")
            return f"≈{usd_amount:.2f} USD equivalent"
        
    
        # ─── Verification helpers ────────────────────────────────────────────────
    def _parse_expected_amount(self, invoice: dict) -> float:
        """
        crypto_amount is stored like:
        '20.15 USDT', '0.00012345 BTC', '0.45 SOL'
        This extracts the numeric part.
        """
    ...
    async def verify_blockcypher(self, invoice: dict, db, chain: str) -> tuple[bool, str | None]:
        ...
        for tx in data.get("txs", []):
            ...
            # confirmations > 0 means mined/confirmed
            ...
            for output in tx.get("outputs", []):
                ...
                # BTC/LTC value is in satoshis/litoshis
